...
短信内容要求和哈希字符串的生成请参考：
https://developers.google.com/identity/sms-retriever/verify?hl=zh-cn#computing_your_apps_hash_string
插件中也提供了获取hash的方法 getAppSignatures 打印回调数据查看日志（注意：此方法获取的hash值和app的签名文件有关，使用时请配置正确的签名文件获取值，
不推荐使用，更推荐大家通过上面的官方说明获取正确的hash值）
...
插件名称：
1.引用原生插件
const automaticSms = uni.requireNativePlugin('AutomaticSms');
2.页面创建后开始监听
automaticSms.registerSmsObserver(
					(ret) => {
...
3.页面销毁前注销监听
automaticSms.unRegisterSmsObserver();
4.调用接口发送验证码成功后
automaticSms.startSmsRetriever(
					(ret) => {
